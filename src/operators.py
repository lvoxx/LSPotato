...
class GenerateLinkedGraphOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            result = subprocess.run(['dot', '-Tpng', dot_path, '-o', png_path], capture_output=True)

            if result.returncode == 0:
                # Load the image into Blender
                img = bpy.data.images.load(png_path, check_existing=True)
                self.report({'INFO'}, f"Graph image loaded: {img.name}. View in Image Editor.")
                # Optionally clean up
                os.remove(dot_path)
                os.remove(png_path)
            else:
                self.report({'WARNING'}, f"Graphviz not found or failed. DOT file saved at: {dot_path}")
