# ============================================================
# AUTO-GENERATED by LSPotato NodeCompiler
# Do NOT edit manually — regenerate from the source .blend file.
# ============================================================

import bpy  # type: ignore
from mathutils import Color, Euler, Matrix, Quaternion, Vector  # type: ignore
from ......node import ShaderNode, ensure_node_group


class ShaderNodeCompiled_HSR__Seperate_Body_Lightmap(ShaderNode):
    bl_idname = 'ShaderNodeCompiled_HSR__Seperate_Body_Lightmap'
    bl_label = 'lscherry.external.michos.honkai_star_rail.HSR: Seperate Body Lightmap'
    bl_icon = "NONE"
    _PREFIX = "."

    def draw_label(self):
        return 'HSR: Seperate Body Lightmap'

    def init(self, context):
        self.getNodetree(self.name + '_node_tree')
        self.inputs['Lightmap'].default_value = (1.0, 1.0, 1.0, 1.0)

    def createNodetree(self, name):
        # Use bl_label as a stable, class-level key so all instances share
        # one node tree and nested references resolve correctly.
        nt = self.node_tree = bpy.data.node_groups.new(
            self._PREFIX + self.bl_label, 'ShaderNodeTree'
        )
        nt.color_tag = 'CONVERTER'

        _sock_out_Shadow = nt.interface.new_socket(name='Shadow', in_out='OUTPUT', socket_type='NodeSocketFloat')
        _sock_out_Shadow.default_value = 0.0
        _sock_out_Shadow.min_value = -3.4028234663852886e+38
        _sock_out_Shadow.max_value = 3.4028234663852886e+38
        _sock_out_Metal = nt.interface.new_socket(name='Metal', in_out='OUTPUT', socket_type='NodeSocketFloat')
        _sock_out_Metal.default_value = 0.0
        _sock_out_Metal.min_value = -3.4028234663852886e+38
        _sock_out_Metal.max_value = 3.4028234663852886e+38
        _sock_inp_Lightmap = nt.interface.new_socket(name='Lightmap', in_out='INPUT', socket_type='NodeSocketColor')
        _sock_inp_Lightmap.default_value = (1.0, 1.0, 1.0, 1.0)
        _sock_inp_Lightmap.hide_value = True

        Group_Input = nt.nodes.new('NodeGroupInput')
        Group_Input.location = (-677.05, -98.67)

        Group = nt.nodes.new('ShaderNodeGroup')
        Group.location = (-410.75, -17.87)
        Group.node_tree = ensure_node_group('.lscherry.utils.separator.Seperate Lightmap')

        Group_Output = nt.nodes.new('NodeGroupOutput')
        Group_Output.location = (1149.5, -25.54)
        Group_Output.is_active_output = True

        Map_Range = nt.nodes.new('ShaderNodeMapRange')
        Map_Range.location = (154.4, 317.75)
        Map_Range.clamp = True
        Map_Range.interpolation_type = 'LINEAR'
        Map_Range.data_type = 'FLOAT'
        Map_Range.inputs[1].default_value = 0.8999999761581421
        Map_Range.inputs[2].default_value = 1.0
        Map_Range.inputs[3].default_value = 0.0
        Map_Range.inputs[4].default_value = 1.0
        Map_Range.inputs[5].default_value = 4.0
        Map_Range.inputs[6].default_value = (0.0, 0.0, 0.0)
        Map_Range.inputs[7].default_value = (0.0, 0.0, 0.0)
        Map_Range.inputs[8].default_value = (1.0, 1.0, 1.0)
        Map_Range.inputs[9].default_value = (0.0, 0.0, 0.0)
        Map_Range.inputs[10].default_value = (1.0, 1.0, 1.0)
        Map_Range.inputs[11].default_value = (4.0, 4.0, 4.0)

        Map_Range_001 = nt.nodes.new('ShaderNodeMapRange')
        Map_Range_001.location = (143.48, -268.28)
        Map_Range_001.clamp = True
        Map_Range_001.interpolation_type = 'LINEAR'
        Map_Range_001.data_type = 'FLOAT'
        Map_Range_001.inputs[1].default_value = 0.009999999776482582
        Map_Range_001.inputs[2].default_value = 1.0
        Map_Range_001.inputs[3].default_value = 0.0
        Map_Range_001.inputs[4].default_value = 1.0
        Map_Range_001.inputs[5].default_value = 4.0
        Map_Range_001.inputs[6].default_value = (0.0, 0.0, 0.0)
        Map_Range_001.inputs[7].default_value = (0.0, 0.0, 0.0)
        Map_Range_001.inputs[8].default_value = (1.0, 1.0, 1.0)
        Map_Range_001.inputs[9].default_value = (0.0, 0.0, 0.0)
        Map_Range_001.inputs[10].default_value = (1.0, 1.0, 1.0)
        Map_Range_001.inputs[11].default_value = (4.0, 4.0, 4.0)

        Mix = nt.nodes.new('ShaderNodeMix')
        Mix.location = (388.6, 181.66)
        Mix.data_type = 'FLOAT'
        Mix.factor_mode = 'UNIFORM'
        Mix.blend_type = 'MIX'
        Mix.clamp_factor = True
        Mix.clamp_result = False
        Mix.inputs[1].default_value = (0.5, 0.5, 0.5)
        Mix.inputs[3].default_value = 1.0
        Mix.inputs[4].default_value = (0.0, 0.0, 0.0)
        Mix.inputs[5].default_value = (0.0, 0.0, 0.0)
        Mix.inputs[6].default_value = (0.5, 0.5, 0.5, 1.0)
        Mix.inputs[7].default_value = (0.5, 0.5, 0.5, 1.0)
        Mix.inputs[8].default_value = (0.0, 0.0, 0.0)
        Mix.inputs[9].default_value = (0.0, 0.0, 0.0)

        Math_001 = nt.nodes.new('ShaderNodeMath')
        Math_001.location = (609.0, 158.89)
        Math_001.operation = 'MULTIPLY'
        Math_001.use_clamp = False
        Math_001.inputs[1].default_value = 10.0
        Math_001.inputs[2].default_value = 0.5

        Invert_Color = nt.nodes.new('ShaderNodeInvert')
        Invert_Color.location = (882.74, 77.96)
        Invert_Color.inputs[0].default_value = 1.0

        Math_002 = nt.nodes.new('ShaderNodeMath')
        Math_002.location = (330.13, -271.11)
        Math_002.operation = 'MULTIPLY'
        Math_002.use_clamp = False
        Math_002.inputs[1].default_value = 10.0
        Math_002.inputs[2].default_value = 0.5

        Mix_001 = nt.nodes.new('ShaderNodeMix')
        Mix_001.location = (762.28, -50.01)
        Mix_001.data_type = 'FLOAT'
        Mix_001.factor_mode = 'UNIFORM'
        Mix_001.blend_type = 'MIX'
        Mix_001.clamp_factor = True
        Mix_001.clamp_result = False
        Mix_001.inputs[1].default_value = (0.5, 0.5, 0.5)
        Mix_001.inputs[2].default_value = 0.0
        Mix_001.inputs[4].default_value = (0.0, 0.0, 0.0)
        Mix_001.inputs[5].default_value = (0.0, 0.0, 0.0)
        Mix_001.inputs[6].default_value = (0.5, 0.5, 0.5, 1.0)
        Mix_001.inputs[7].default_value = (0.5, 0.5, 0.5, 1.0)
        Mix_001.inputs[8].default_value = (0.0, 0.0, 0.0)
        Mix_001.inputs[9].default_value = (0.0, 0.0, 0.0)

        Math = nt.nodes.new('ShaderNodeMath')
        Math.location = (555.85, -271.86)
        Math.operation = 'GREATER_THAN'
        Math.use_clamp = False
        Math.inputs[1].default_value = 0.10000000149011612
        Math.inputs[2].default_value = 0.5


        nt.links.new(Group_Input.outputs['Lightmap'], Group.inputs['Lightmap'])
        nt.links.new(Invert_Color.outputs['Color'], Group_Output.inputs['Shadow'])
        nt.links.new(Mix_001.outputs[0], Group_Output.inputs['Metal'])
        nt.links.new(Group.outputs['Metal'], Map_Range.inputs['Value'])
        nt.links.new(Group.outputs['Highlight'], Map_Range_001.inputs['Value'])
        nt.links.new(Map_Range.outputs['Result'], Mix.inputs[0])
        nt.links.new(Group.outputs['Metal'], Mix.inputs[2])
        nt.links.new(Mix.outputs[0], Math_001.inputs[0])
        nt.links.new(Math_001.outputs['Value'], Invert_Color.inputs['Color'])
        nt.links.new(Map_Range_001.outputs['Result'], Math_002.inputs[0])
        nt.links.new(Math.outputs['Value'], Mix_001.inputs[0])
        nt.links.new(Math_002.outputs['Value'], Mix_001.inputs[3])
        nt.links.new(Math_002.outputs['Value'], Math.inputs[0])
