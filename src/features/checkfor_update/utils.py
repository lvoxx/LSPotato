...
def version_to_tuple(version_string, length=3):
    ...
    if not isinstance(version_string, str):
        raise VersionComparisonException(
            str(version_string), 
            "N/A",
            "Version string must be a string type"
        )
    ...
    try:
        ...
        raise VersionComparisonException(
            version_string,
            "N/A", 
            f"Failed to parse version string: {str(e)}"
        )
...
def normalize_version_tuple(version, length=3):
    ...
    if not isinstance(version, (tuple, list)):
        raise VersionComparisonException(
            str(version),
            "N/A",
            "Version must be a tuple or list"
        )
...
def check_for_updates():
    ...
    try:
        ...
        # If it's just a skip in minor versions, consider as major
            ...
            result['has_update'] = True
            result['major_update'] = latest_tag
            logger.info(f"Major minor update available (skipped versions): {latest_tag}")
...
def download_and_install_update(version_tag=None):
    ...
    try:
        ...
        if version_tag:
            ...
            download_url = f"https://github.com/lvoxx/LSPotato/archive/refs/tags/{version_tag}.zip"
            logger.info(f"Downloading specific version: {version_tag}")
        else:
            # Download latest from main
            download_url = GITHUB_DOWNLOAD_URL
            logger.info("Downloading latest version from main branch")
        
        logger.info(f"Download URL: {download_url}")
        
        # Create temp directory
        temp_dir = tempfile.mkdtemp()
        zip_filename = f"LSPotato-{version_tag if version_tag else 'main'}.zip"
        ...
        try:
            ...
            raise FileSystemException(
                "Write",
                zip_path,
                f"Failed to save zip file: {str(e)}"
            )
        ...
        if not root_dir:
            raise ExtractionException(
                zip_path,
                "Could not find LSPotato root directory in downloaded files"
            )
        ...
        if os.path.exists(src_dir):
            ...
            try:
                ...
                raise FileSystemException(
                    "Copy",
                    src_dir,
                    f"Failed to copy files from src: {str(e)}"
                )
            ...
            if os.path.exists(init_file):
                try:
                    ...
                    raise FileSystemException(
                        "Copy",
                        init_file,
                        f"Failed to copy __init__.py: {str(e)}"
                    )
            else:
            ...
            try:
                ...
                raise FileSystemException(
                    "Copy",
                    root_dir,
                    f"Failed to copy root directory: {str(e)}"
                )
